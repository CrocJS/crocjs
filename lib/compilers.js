...
module.exports = function(app) {
    ...
    app.compilers['.tpl'] = function(file) {
        var widgetToken = '<widget';
        var widgetEndToken = '</widget>';
        var widgetsStack = [];
        var widgetTag = false;
        for (var i = 0; i < file.length; ++i) {
            if (widgetTag) {
                ...
                if (file.substr(i, widgetToken.length) === widgetToken) {
                    widgetsStack.push(file.substr(i).match(/\bis\s*=\s*(["'])(.*?)\1/)[2]);
                    widgetTag = true;
                }
                else if (file.substr(i, widgetEndToken.length) === widgetEndToken) {
                    widgetsStack.pop();
                }
            }
            else if (widgetsStack.length && file[i] === 'i') {
                file = file.replace(new RegExp('^([\\s\\S]{' + i + '}\\bis\\s*=\\s*(["\']))(:.*?)(\\2)'),
                    '$1' + _.last(widgetsStack) + '$3$4');
            }
        }
