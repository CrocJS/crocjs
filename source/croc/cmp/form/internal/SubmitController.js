 ...
 */
croc.Class.define('croc.cmp.form.internal.SubmitController', {
    extend: croc.Object,
    ...
    options: {
         ...
         * @type {function(this: croc.cmp.form.Form, croc.cmp.form.Form, Object.<string, *>):$.Deferred}
         */
        ajaxSubmitFn: null,
    ...
    construct: function(options) {
        croc.cmp.form.internal.SubmitController.superclass.construct.apply(this, arguments);
    ...
    members: {
        ...
        setAjaxSubmitDelay: function(delay) {
            this._options.ajaxSubmitDelay = delay;
        },
        ...
        submit: function() {
            ...
            this.__form.commitChanges();
            
            var prevented = false;
            this.__form.fireEvent('presubmit', function() { prevented = true; });
            if (prevented) {
            ...
            this.__form.validate().done(function(valid) {
                if (valid) {
                    this.__enableSubmit = true;
                    this.__form.getElement().submit();
                    this.__form.fireEvent('postsubmit');
                    this.__enableSubmit = false;
                }
                else {
                    this.__form.fireEvent('submitValidationFail');
        ...
        __ajaxSubmit: function(form) {
            ...
            //var values = this._options.valuesFunc();
            //var showMessages = true;
            //return stm.ajax({
            //    url: this._options.action,
            //    data: this._options.ajaxAction ? [
            //        {action: this._options.ajaxAction, params: values}
            //    ] : values,
            //    delayRequest: this._options.ajaxSubmitDelay
            //}, this)
            //    
            //    .then(function(response) {
            //        showMessages = this._options.submitSuccessFunc(response, values);
            //        if (showMessages !== false) {
            //            showMessages = true;
            //        }
            //        return response;
            //    }.bind(this), this._options.submitFailFunc.bind(this))
            //    
            //    .then(
            //    function(response) {
            ...
            //                (this._options.errorCodes && this._options.errorCodes[response.errcode]);
            //                
            //                if (error) {
            //                    if (typeof error === 'string') {
            //                        this.__form.getValidationManager().setInvalidItem(this.__form, error);
            //                    }
            //                    else {
            //                        this.__form.getValidationManager()
            //                            .setInvalidItem(this.__form.getItem(error.field), error.message);
            ...
            //                        var field = this.__form.getItem(name);
            //                        if (message && field) {
            //                            this.__form.getValidationManager().setInvalidItem(field, message);
            //                        }
            //                    }, this);
            //                    
            //                    if (!error) {
        ...
        __onFormElSubmit: function(e) {
            if (!this.__enableSubmit) {
                ...
                this.__form.fireEvent('submit', e, this.__form.getValues());
                ...
                if (this._options.preventSubmit || this._options.ajaxSubmitFn || this._options.autoAjaxSubmit) {
                    e.preventDefault();
                }
                else {
                ...
                var submitFn = this._options.ajaxSubmitFn;
                if (!submitFn && this._options.autoAjaxSubmit) {
                    submitFn = this.__ajaxSubmit;
                }
                
                if (submitFn) {
                    submitFn.call(this, this.__form, this.__form.getValues())
                        .then(
                        function(error, response) {
                            this.__submitUnblock();
                            if (error) {
                                this.__submitDeferred.reject();
                                this.__form.fireEvent('submitServerError', error);
                            }
                            else {
                        ...
                        function(response) {
                            this.__submitUnblock();
                            this.__submitDeferred.reject();
                            this.__form.fireEvent('submitServerError',
                                'errcode = ' + (response && response.status));
                        }.bind(this));
                }
        ...
        __preprocessFields: function() {
            this.__form.getItems().forEach(function(field) {
                
                if (this._options.trimAllBeforeSubmit &&
                    croc.Interface.check(field, 'croc.cmp.form.field.ITextField') && !field.keepWhiteSpace() &&
        ...
        __submitSuccessful: function(isStatic, response) {
            this.__form.getValidationManager().resetValidation();
            this.__submitDeferred.resolve();
            this.__form.fireEvent('submitSuccessful', response, isStatic);
        },
